...
if (strcmp($res, "VERIFIED") == 0) {
	...
	if ($isPaymentCompleted) {
		$param_value_array = array("userName" => $user_name,  "PaymentStatus" => $payment_status, "txnId" => $txn_id, "payment_amount" => $payment_amount);
		$orderId = $db->query("INSERT INTO orders (userName,PaymentStatus,txnId,paymentAmount,orderStatus,products
		                          VALUES(:userName,:PaymentStatus,:txnId,:paymentAmount,:orderStatus,:products);
								  SELECT LAST_INSERT_ID();", $param_value_array);
		$products = json_decode($products);
		$param_value_array=array("orderId"=>$orderId);
		foreach ($products as $id => $quantity) {
			$param_value_array["productStockId"]=$productStockId;
